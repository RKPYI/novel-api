...
class ImageUploadHelper
{
    ...
    /**
     * Upload and process a novel asset image
     * Saves to: /novels/:novel-slug/assets/:hash.webp
     *
     * @param UploadedFile $file
     * @param string $novelSlug
     * @return array Metadata about the uploaded asset
     */
    public static function uploadNovelAsset(UploadedFile $file, string $novelSlug): array
    {
        // Validate image
        self::validateImage($file);

        // Generate unique filename
        $hash = hash('sha256', $novelSlug . now()->timestamp . Str::random(16));
        $directory = "novels/{$novelSlug}/assets";
        $filename = "{$hash}.webp";
        $fullPath = "{$directory}/{$filename}";

        // Process and optimize image (max 1920x1080, maintain aspect ratio)
        $processedImage = self::processImage($file, 1920, 1080);

        // Store the processed image
        Storage::disk('public')->put($fullPath, $processedImage, 'public');
    ...
    /**
     * Delete a single novel asset file from disk
     *
     * @param string $assetPath The URL path like /storage/novels/slug/assets/hash.webp
     * @return bool
     */
    public static function deleteNovelAsset(string $assetPath): bool
    {
        $path = str_replace('/storage/', '', $assetPath);
