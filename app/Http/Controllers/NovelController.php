...
class NovelController extends Controller
{
    ...
    /**
     * Upload an asset image for a novel.
     */
    public function uploadAsset(Request $request, string $slug)
    {
        ...
        if ($novel->user_id !== $request->user()->id && !$request->user()->isAdmin()) {
            return response()->json([
                'message' => 'You can only upload assets for your own novels'
            ], 403);
        }

        $request->validate([
            'image' => 'required|image|mimes:jpeg,png,jpg,gif,webp|max:20480', // 20MB max (auto-compressed)
        ]);

        try {
            $metadata = ImageUploadHelper::uploadNovelAsset(
                $request->file('image'),
                $novel->slug
            );

            $asset = NovelAsset::create([
                'novel_id' => $novel->id,
    ...
    /**
     * List all assets for a novel.
     */
    public function listAssets(Request $request, string $slug)
    {
        $novel = Novel::where('slug', $slug)->first();
        ...
        // Check ownership (only owner or admin can view assets)
        if ($novel->user_id !== $request->user()->id && !$request->user()->isAdmin()) {
            return response()->json([
                'message' => 'You can only view assets for your own novels'
            ], 403);
        }

        $assets = $novel->assets()->orderBy('created_at', 'desc')->get();

        return response()->json([
            'message' => 'Novel assets',
            'assets' => $assets
        ], 200);
    }

    /**
     * Delete a novel asset.
     */
    public function deleteAsset(Request $request, string $slug, NovelAsset $asset)
    {
        $novel = Novel::where('slug', $slug)->first();
        ...
        // Verify the asset belongs to this novel
        if ($asset->novel_id !== $novel->id) {
            return response()->json([
                'message' => 'Asset not found for this novel'
            ], 404);
        }
        ...
        if ($novel->user_id !== $request->user()->id && !$request->user()->isAdmin()) {
            return response()->json([
                'message' => 'You can only delete assets for your own novels'
            ], 403);
        }

        try {
            // Delete file from disk
            ImageUploadHelper::deleteNovelAsset($asset->getRawOriginal('path'));
